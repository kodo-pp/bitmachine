...
use crate::ast::{Program, FunctionMap, Function, FunctionVariant, Expr};
use crate::bitstring::Bit;
use crate::pattern::{MultiPattern, Pattern, VarLenPattern, ConstLenPattern, ConstLenPatternElement};
use pest::Parser;
...
fn parse_expr_cat(expr: Pair<'_>) -> Expr {
    ...
    Expr::Cat { children: expr.into_inner().map(parse_expr_atomic).collect() }
}
...
fn parse_const_len_pattern(pattern: Pair<'_>) -> ConstLenPattern {
    ...
    ConstLenPattern { elements: pattern.into_inner().map(parse_const_len_pattern_item).collect() }
}
